(() => {
  ...
  function surveyFieldLabel(key) {
    return ({
      ...
      issue: 'Problème rencontré', status: 'Statut', private_note: 'Note privée'
  ...
  function renderAdminResponseCard(item, source = 'survey') {
    ...
    const details = Object.entries(item).filter(([key]) => !ignored.has(key)).map(([key, value]) => `
      <div class="admin-answer"><span>${escapeHtml(surveyFieldLabel(key))}</span><strong>${escapeHtml(readableSurveyValue(value))}</strong></div>`).join('');
    const status = item.status || 'nouveau';
    return `<article class="card admin-response-card">
      <div class="admin-response-heading"><div><p class="eyebrow">${source === 'feedback' ? 'Avis utilisateur' : 'Questionnaire'}</p><h3>${escapeHtml(adminDate(item.created_at))}</h3></div><span class="admin-status status-${escapeHtml(status)}">${escapeHtml(status)}</span></div>
      <div class="admin-answer-grid">${details || '<p class="muted">Aucune réponse détaillée.</p>'}</div>
      ${item.private_note ? `<div class="admin-private-note"><strong>Note privée</strong><p>${escapeHtml(item.private_note)}</p></div>` : ''}
      <div class="admin-response-actions">
        <select aria-label="Statut du retour" data-admin-status-source="${source}" data-admin-id="${escapeHtml(item.id)}">
          ${['nouveau','a_etudier','prevu','traite'].map((value) => `<option value="${value}" ${status === value ? 'selected' : ''}>${value === 'a_etudier' ? 'À étudier' : value.charAt(0).toUpperCase() + value.slice(1)}</option>`).join('')}
        </select>
        <button type="button" class="text-button danger-text-button" data-action="admin-delete-response" data-admin-source="${source}" data-admin-id="${escapeHtml(item.id)}">Supprimer</button>
      </div>
    </article>`;
  }
  ...
  function renderAdmin() {
    ...
    const average = adminFeedback.length ? (adminFeedback.reduce((sum, item) => sum + Number(item.rating || 0), 0) / adminFeedback.filter((item) => Number(item.rating)).length || 0) : 0;
    const interested = adminResponses.filter((item) => /oui|yes|interess/i.test(readableSurveyValue(item.testing_interest))).length;
    return `<section class="page-heading admin-heading"><div><p class="eyebrow">Espace privé</p><h1>Retours utilisateurs</h1><p>Questionnaires, avis et demandes regroupés dans un seul endroit.</p></div><button type="button" class="secondary-button" data-action="admin-refresh">Actualiser</button></section>
      <section class="admin-stats">
        <article class="card"><span>Questionnaires</span><strong>${adminResponses.length}</strong></article>
        <article class="card"><span>Intéressés par les tests</span><strong>${interested}</strong></article>
        <article class="card"><span>Avis reçus</span><strong>${adminFeedback.length}</strong></article>
        <article class="card"><span>Note moyenne</span><strong>${average ? `${average.toFixed(1)}/5` : '—'}</strong></article>
      </section>
      ${adminLoading ? '<div class="card card-pad admin-message">Chargement des retours…</div>' : ''}
      ${adminError ? `<div class="card card-pad admin-error">${escapeHtml(adminError)}</div>` : ''}
      <section class="admin-section"><div class="home-section-heading"><div><p class="eyebrow">Questionnaire public</p><h2>Dernières réponses</h2></div></div>
        <div class="admin-response-list">${adminResponses.length ? adminResponses.map((item) => renderAdminResponseCard(item, 'survey')).join('') : '<div class="card card-pad admin-message">Aucune réponse pour le moment.</div>'}</div>
      </section>
      <section class="admin-section"><div class="home-section-heading"><div><p class="eyebrow">Notes et commentaires</p><h2>Avis des testeurs</h2></div></div>
        <div class="admin-response-list">${adminFeedback.length ? adminFeedback.map((item) => renderAdminResponseCard(item, 'feedback')).join('') : '<div class="card card-pad admin-message">Aucun avis enregistré pour le moment.</div>'}</div>
      </section>`;
  }
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'admin-refresh') { adminResponses = []; adminFeedback = []; await loadAdminData(); }
