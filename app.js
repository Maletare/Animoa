(() => {
  ...
  function renderAdminResponseCard(item, source = 'survey') {
    ...
    return `<details class="admin-accordion" data-admin-entry="${source}">
      ...
      ${source === 'access' && item.email ? `<div class="admin-invite-box"><div class="admin-invite-heading"><strong>Envoyer l’accès</strong><span>Le code n’est pas enregistré dans Animoa.</span></div><label for="admin-invite-${escapeHtml(item.id)}">Code d’invitation</label><input id="admin-invite-${escapeHtml(item.id)}" type="text" autocomplete="off" autocapitalize="characters" spellcheck="false" data-admin-invite-id="${escapeHtml(item.id)}" placeholder="Saisissez le code à transmettre"><div class="admin-invite-actions"><button type="button" class="primary-button" data-action="admin-accept-email" data-admin-id="${escapeHtml(item.id)}">Accepter et préparer l’e-mail</button><button type="button" class="secondary-button" data-action="admin-copy-invite" data-admin-id="${escapeHtml(item.id)}">Copier le message</button></div></div>` : ''}
  ...
  async function prepareAccessInvitation(id, mode = 'email') {
    ...
    const { error } = await adminClient().from('animoa_access_requests').update({ status: 'accepte', updated_at: new Date().toISOString() }).eq('id', id);
    if (error) return showToast(error.message || 'Acceptation impossible.');
    item.status = 'accepte';
    const subject = 'Votre accès à Animoa';
    const mailto = `mailto:${encodeURIComponent(item.email)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`;
    showToast('Demande acceptée. Ouverture de votre messagerie…');
    window.location.href = mailto;
    setTimeout(() => { if (currentPage === 'admin') renderPage(); }, 500);
  }
