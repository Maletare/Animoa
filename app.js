(() => {
  ...
  function renderAdminResponseCard(item, source = 'survey') {
    ...
    return `<details class="admin-accordion" data-admin-entry="${source}">
      ...
      ${source === 'access' && item.email ? `<div class="admin-invite-box" data-admin-invite-panel="${escapeHtml(item.id)}"><div class="admin-invite-heading"><strong>Répondre à cette demande</strong><span>Saisissez le code, puis envoyez l’accès directement par e-mail.</span></div><label for="admin-invite-${escapeHtml(item.id)}">Code d’invitation</label><input id="admin-invite-${escapeHtml(item.id)}" type="text" autocomplete="off" autocapitalize="none" spellcheck="false" data-admin-invite-id="${escapeHtml(item.id)}" placeholder="Ex. ANIMOA-XXXX"><div class="admin-invite-actions"><button type="button" class="primary-button admin-email-button" data-action="admin-accept-email" data-admin-id="${escapeHtml(item.id)}">Accepter et envoyer l’accès</button><button type="button" class="secondary-button" data-action="admin-copy-invite" data-admin-id="${escapeHtml(item.id)}">Copier le message</button></div><small class="admin-invite-security">Le code n’est pas enregistré dans Animoa.</small></div>` : ''}
  ...
  async function prepareAccessInvitation(id, mode = 'email') {
    ...
    try {
      ...
      if (error) throw new Error(data?.error || error.message || 'L’e-mail n’a pas pu être envoyé.');
      if (!data?.success) throw new Error(data?.error || 'L’e-mail n’a pas pu être envoyé.');
