(() => {
  ...
  const RELEASE_NOTES = Object.freeze({
    ...
    subtitle: 'Les liens TikTok et Facebook sont maintenant plus lisibles.',
    items: [
      'Le contraste des textes et des icônes a été renforcé sur la page d’accueil publique.',
      'Les liens restent accessibles discrètement dans « Aide et contact » dans les paramètres.',
      'Aucun changement n’est nécessaire dans vos habitudes d’utilisation.'
    ]
  });

  const HEALTH_GROUPS = Object.freeze([
    { id: 'all', label: 'Tout', title: 'Tout le dossier', icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 6h11M9 12h11M9 18h11"/><circle cx="4.5" cy="6" r="1"/><circle cx="4.5" cy="12" r="1"/><circle cx="4.5" cy="18" r="1"/></svg>', types: null },
    { id: 'agenda', label: 'Agenda', title: 'Agenda', icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="5" width="18" height="16" rx="3"/><path d="M8 3v4M16 3v4M3 10h18"/><path d="M8 14h2M14 14h2M8 18h2M14 18h2"/></svg>', types: ['Vaccin', 'Rendez-vous'] },
    { id: 'care', label: 'Soins', title: 'Soins', icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3v5a6 6 0 0 0 12 0V3"/><path d="M9 3v4a3 3 0 0 0 6 0V3M12 14v2a5 5 0 0 0 5 5h1"/><circle cx="20" cy="18" r="2"/></svg>', types: ['Traitement', 'Médicament', 'Analyse', 'Autre'] },
    { id: 'documents', label: 'Docs', title: 'Documents', icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 2h8l4 4v16H6z"/><path d="M14 2v5h5M9 12h6M9 16h6"/></svg>', types: ['Document'] }
  ]);
  const HEALTH_GROUP_IDS = new Set(HEALTH_GROUPS.map((group) => group.id));
  ...
  let reviewPromptClaim = null;
  let reviewPromptSuppressedUntil = 0;
  let reviewPromptMode = 'automatic';
  let reviewDraftRating = 0;
  let reviewDraftComment = '';
  let publicReviewConfig = null;
  let publicReviewConfigPromise = null;
  let internalReviewSummary = null;
  let internalReviewSummaryPromise = null;
  let myInternalReview = null;
  let myInternalReviewLoaded = false;
  ...
  function healthGroupForType(type) {
    const normalizedType = normalizeHealthType(type);
    return HEALTH_GROUPS.find((group) => Array.isArray(group.types) && group.types.includes(normalizedType))
      || HEALTH_GROUPS.find((group) => group.id === 'care');
  }

  function resolveHealthGroup(filter) {
    const direct = HEALTH_GROUPS.find((group) => group.id === filter);
    if (direct) return direct;
    if (filter && filter !== 'Tous') return healthGroupForType(filter);
    return HEALTH_GROUPS[0];
  }
    ...
    const normalized = {
      ...
      health: Array.isArray(migrated.health) ? migrated.health.map((item) => {
        ...
        return {
          ...item,
          type,
          time: isDocument ? '' : (validTime(item.time) ? item.time : ''),
          status: isDocument ? 'done' : (item.status === 'done' ? 'done' : 'planned'),
          reminder: isDocument ? false : Boolean(item.reminder),
          offerCalendar: isDocument ? false : item.offerCalendar,
  ...
  function nextReminder() {
    return petItems('health')
      .filter((item) => normalizeHealthType(item.type) !== 'Document' && item.status === 'planned' && item.date >= todayIso() && !isPlannedHealthOverdue(item))
      .sort((a, b) => healthDateTimeKey(a).localeCompare(healthDateTimeKey(b)))[0] || null;
  }

  function upcomingReminders() {
    return petItems('health')
      .filter((item) => normalizeHealthType(item.type) !== 'Document' && item.status === 'planned' && item.reminder && item.date >= todayIso() && !isPlannedHealthOverdue(item))
  ...
  function recordMenuIcon() {
    return '<svg class="record-menu-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><circle cx="5" cy="12" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg>';
  }
  ...
  function reviewFeatureUnavailable(error) {
    const message = String(error?.message || '');
    return /does not exist|relation .* does not exist|function .* does not exist|schema cache/i.test(message);
  }

  function reviewPromptSuppressionKey() {
    const userId = String(window.AnimoaAuth?.getUser?.()?.id || '').trim();
    return userId ? `animoa_review_prompt_suppression_v1_${userId}` : '';
  }

  function storedReviewPromptSuppression() {
    const key = reviewPromptSuppressionKey();
    if (!key) return null;
    try {
      ...
      return { until, reason: String(value?.reason || '') };
    } catch {
      return null;
    }
  }

  function reviewPromptIsLocallySuppressed() {
    const stored = storedReviewPromptSuppression();
    return Math.max(reviewPromptSuppressedUntil, Number(stored?.until || 0)) > Date.now();
  }

  function suppressReviewPrompt(reason, until) {
    const safeUntil = Math.max(Date.now() + 60_000, Number(until || 0));
    reviewPromptSuppressedUntil = Math.max(reviewPromptSuppressedUntil, safeUntil);
    reviewPromptActive = false;
    clearTimeout(reviewPromptRetryTimer);
    reviewPromptRetryTimer = null;
    const key = reviewPromptSuppressionKey();
    if (!key) return;
    try {
      localStorage.setItem(key, JSON.stringify({ reason: String(reason || ''), until: safeUntil }));
    } catch {}
  }

  function reviewPromptSuppressionUntil(action) {
    const day = 24 * 60 * 60 * 1000;
    if (action === 'later') {
      const promptCount = Math.max(1, Number(reviewPromptClaim?.prompt_count || 1));
      const days = promptCount <= 1 ? 7 : (promptCount === 2 ? 30 : 3650);
      return Date.now() + (days * day);
    }
    if (action === 'never' || action === 'submitted' || action === 'existing-review') return Date.now() + (3650 * day);
    return Date.now() + (60 * 60 * 1000);
  }

  function queueReviewPromptCheck(delay = 900) {
    clearTimeout(reviewPromptRetryTimer);
    reviewPromptRetryTimer = null;
    if (reviewPromptIsLocallySuppressed()) return;
    reviewPromptRetryTimer = setTimeout(() => {
      reviewPromptRetryTimer = null;
      ensureAndMaybeShowReviewPrompt().catch((error) => console.warn('Demande d’avis indisponible', error));
    }, delay);
  }

  function reviewRatingButtons(selected = 0) {
    return Array.from({ length: 5 }, (_, index) => {
      const value = index + 1;
      const active = value <= selected;
      return `<button type="button" class="review-rating-star ${active ? 'is-active' : ''}" data-action="review-select-rating" data-review-rating="${value}" aria-label="${value} étoile${value > 1 ? 's' : ''}" aria-pressed="${value === selected ? 'true' : 'false'}">★</button>`;
    }).join('');
  }
  ...
  function reviewPromptBody() {
    ...
    return `<div class="review-request-modal internal-review-modal">
      <div class="review-request-icon" aria-hidden="true">★</div>
      <h3>Comment trouvez-vous Animoa ?</h3>
      <p>Votre retour nous aide à améliorer l’application. La note moyenne pourra être affichée aux utilisateurs, mais votre commentaire restera privé et visible uniquement par l’administrateur.</p>
      <div class="review-rating-field">
        <span class="review-rating-label">Votre note *</span>
        <div class="review-rating-picker" role="group" aria-label="Choisir une note sur 5">${reviewRatingButtons(reviewDraftRating)}</div>
        <input id="internalReviewRating" type="hidden" value="${reviewDraftRating || ''}">
        <small id="internalReviewRatingHint">${reviewDraftRating ? `${reviewDraftRating} sur 5` : 'Touchez une étoile pour choisir votre note.'}</small>
      </div>
      <label class="review-comment-field" for="internalReviewComment"><span>Votre commentaire <small>(facultatif)</small></span><textarea id="internalReviewComment" maxlength="1200" rows="4" placeholder="Ce que vous aimez, ce qui pourrait être amélioré…">${comment}</textarea><small>Visible uniquement dans l’Administration Animoa.</small></label>
      <div class="review-request-actions ${automatic ? '' : 'is-single-action'}">
  ...
  function showReviewPrompt(_config = null, { mode = 'automatic', existingReview = null } = {}) {
    reviewPromptMode = mode;
    reviewPromptActive = mode === 'automatic';
    reviewDraftRating = Number(existingReview?.rating || 0);
    reviewDraftComment = String(existingReview?.comment || '');
    openModal(existingReview ? 'Modifier mon avis 🐾' : 'Votre avis compte 🐾', reviewPromptBody(), 'Animoa');
  }
  ...
  async function loadPublicReviewConfig({ force = false } = {}) {
    ...
    publicReviewConfigPromise = client
      ...
      .select('enabled,platform,review_url')
      .eq('id', true)
      .maybeSingle()
      .then(({ data: config, error }) => {
        if (error) {
          if (!reviewFeatureUnavailable(error)) console.warn('Configuration des avis inaccessible', error);
          publicReviewConfig = null;
          return null;
        }
        publicReviewConfig = config || null;
        return publicReviewConfig;
      })
      .finally(() => { publicReviewConfigPromise = null; });
    return publicReviewConfigPromise;
  }

  function normalizedReviewSummary(value) {
    const row = rpcRow(value) || {};
    return {
      reviewCount: Math.max(0, Number(row.review_count || 0)),
      averageRating: Math.max(0, Math.min(5, Number(row.average_rating || 0)))
    };
  }
  ...
  async function loadInternalReviewSummary({ force = false } = {}) {
    ...
    internalReviewSummaryPromise = client.rpc('get_animoa_review_summary')
      .then(({ data: summary, error }) => {
        if (error) {
          if (!reviewFeatureUnavailable(error)) console.warn('Résumé des avis inaccessible', error);
          internalReviewSummary = null;
          return null;
        }
        internalReviewSummary = normalizedReviewSummary(summary);
        return internalReviewSummary;
      })
      .finally(() => { internalReviewSummaryPromise = null; });
    return internalReviewSummaryPromise;
  }
  ...
  async function loadMyInternalReview({ force = false } = {}) {
    ...
    myInternalReview = rpcRow(data);
    myInternalReviewLoaded = true;
    return myInternalReview;
  }
  ...
  async function hydrateHomeReviewSummary() {
    const card = document.getElementById('homeReviewSummary');
    if (!card) return;
    const summary = await loadInternalReviewSummary();
    if (!card.isConnected || currentPage !== 'home') return;
    if (!summary || summary.reviewCount < 1) {
      card.hidden = true;
      return;
    }
    card.hidden = false;
    const score = card.querySelector('[data-review-average]');
    const count = card.querySelector('[data-review-count]');
    if (score) score.textContent = `${formatReviewAverage(summary.averageRating)}/5`;
    if (count) count.textContent = `${summary.reviewCount} avis utilisateur${summary.reviewCount > 1 ? 's' : ''}`;
  }

  async function hydrateSettingsReviewCard() {
    const card = document.getElementById('settingsReviewCard');
    if (!card) return;
    ...
    if (!card.isConnected || currentPage !== 'settings') return;
    const button = card.querySelector('[data-action="settings-open-review"]');
    if (button) button.textContent = existing ? 'Modifier mon avis' : 'Donner mon avis';
    const status = card.querySelector('[data-settings-review-status]');
    if (status) {
      status.textContent = existing
        ? `Votre note actuelle : ${Number(existing.rating || 0)}/5.`
        : (summary?.reviewCount ? `Note moyenne : ${formatReviewAverage(summary.averageRating)}/5 sur ${summary.reviewCount} avis.` : 'Soyez parmi les premiers à partager votre expérience.');
    }
  }
  ...
  function selectReviewRating(value) {
    ...
    const input = document.getElementById('internalReviewRating');
    if (input) input.value = String(rating);
    document.querySelectorAll('.review-rating-star').forEach((button) => {
      const buttonRating = Number(button.dataset.reviewRating || 0);
      button.classList.toggle('is-active', buttonRating <= rating);
      button.setAttribute('aria-pressed', buttonRating === rating ? 'true' : 'false');
    });
    const hint = document.getElementById('internalReviewRatingHint');
    if (hint) hint.textContent = `${rating} sur 5`;
  }

  async function submitInternalReview() {
    if (reviewPromptRecording) return;
    const rating = Number(document.getElementById('internalReviewRating')?.value || reviewDraftRating || 0);
    ...
    if (!client) {
      if (automaticPrompt) {
        suppressReviewPrompt('submit-offline', reviewPromptSuppressionUntil('temporary'));
        closeModal();
      }
      return showToast('Connexion indisponible. Vous pourrez réessayer depuis les Paramètres.');
    }
    reviewPromptRecording = true;
    reviewPromptActive = false;
    if (automaticPrompt) suppressReviewPrompt('submitting', reviewPromptSuppressionUntil('temporary'));
    modal.querySelectorAll('[data-action="review-submit"], [data-action="review-later"], [data-action="review-never"]').forEach((button) => { button.disabled = true; });
    try {
      const { data, error } = await client.rpc('submit_animoa_internal_review', { p_rating: rating, p_comment: comment || null });
      if (error) throw error;
      myInternalReview = rpcRow(data) || { rating, comment };
      myInternalReviewLoaded = true;
      internalReviewSummary = null;
      internalReviewSummaryPromise = null;
      suppressReviewPrompt('submitted', reviewPromptSuppressionUntil('submitted'));
      reviewPromptClaim = null;
      closeModal();
      showToast('Merci, votre avis a bien été envoyé.');
      if (currentPage === 'home') requestAnimationFrame(hydrateHomeReviewSummary);
      if (currentPage === 'settings') requestAnimationFrame(hydrateSettingsReviewCard);
    } catch (error) {
      if (!reviewFeatureUnavailable(error)) console.warn('Avis interne non enregistré', error);
      if (automaticPrompt) closeModal();
      else modal.querySelectorAll('[data-action="review-submit"], [data-action="review-later"], [data-action="review-never"]').forEach((button) => { button.disabled = false; });
      showToast(error?.message || 'Votre avis n’a pas pu être enregistré. Vous pourrez réessayer depuis les Paramètres.');
    } finally {
      reviewPromptRecording = false;
    }
  }

  function reviewSummaryBody(summary, { allowReview = false } = {}) {
  ...
  async function openReviewSummary() {
    const [summary, config] = await Promise.all([loadInternalReviewSummary({ force: true }), loadPublicReviewConfig()]);
    if (!summary || summary.reviewCount < 1) return showToast('Aucun avis n’a encore été enregistré.');
    openModal('L’avis des utilisateurs', reviewSummaryBody(summary, { allowReview: String(config?.platform || '').toLowerCase() === 'animoa' }), 'Animoa');
  }

  async function recordReviewAction(action, { close = true, silent = false } = {}) {
    if (reviewPromptRecording) return;
    reviewPromptRecording = true;
    reviewPromptActive = false;
    suppressReviewPrompt(action, reviewPromptSuppressionUntil(action));
    if (close) closeModal();
    ...
    try {
      const { error } = await client.rpc('record_animoa_review_action', { p_action: action });
      if (error) throw error;
      reviewPromptClaim = null;
    } catch (error) {
      if (!reviewFeatureUnavailable(error)) console.warn('Choix de demande d’avis non enregistré ; protection locale conservée', error);
    } finally {
      reviewPromptRecording = false;
    }
  }
  ...
  async function ensureAndMaybeShowReviewPrompt() {
    ...
    try {
      const existingReview = await loadMyInternalReview();
      if (existingReview) {
        suppressReviewPrompt('existing-review', reviewPromptSuppressionUntil('existing-review'));
        return;
      }

      const { error: ensureError } = await client.rpc('ensure_animoa_review_request');
      if (ensureError) {
        if (!reviewFeatureUnavailable(ensureError)) console.warn('Initialisation de la demande d’avis impossible', ensureError);
        return;
      }

      const config = await loadPublicReviewConfig({ force: true });
      if (config?.enabled !== true) return;
      if (modal.classList.contains('open') || (onboardingOverlay && !onboardingOverlay.hidden)) return;

      const { data: claimed, error: claimError } = await client.rpc('claim_animoa_review_prompt');
      if (claimError) {
        if (!reviewFeatureUnavailable(claimError)) console.warn('Demande d’avis impossible', claimError);
        return;
      }
      const claimedRow = rpcRow(claimed);
      if (claimedRow && !reviewPromptIsLocallySuppressed()) {
        reviewPromptClaim = claimedRow;
        showReviewPrompt(config, { mode: 'automatic' });
      }
    } finally {
  ...
  async function searchAdminMedia(form) {
    ...
    try {
      ...
      adminMediaResults = Array.isArray(result.items) ? result.items : [];
      adminMediaWarnings = Array.isArray(result.warnings) ? result.warnings : [];
      if (!adminMediaResults.length) adminMediaSearchError = 'Aucune vidéo adaptée n’a été trouvée. Essayez un autre thème ou un autre format.';
    } catch (error) {
      adminMediaResults = [];
  ...
  async function saveAdminMediaToDrive(key) {
    ...
    try {
      ...
      showToast(result.alreadySaved ? 'Cette vidéo est déjà dans votre médiathèque.' : 'Vidéo ajoutée à votre Google Drive.');
    } catch (error) {
  ...
  function renderAdmin() {
    ...
    return `${secondaryBackButton('home')}<section class="page-heading admin-heading"><div><p class="eyebrow">Espace privé</p><h1>Administration</h1><p>Suivez les inscriptions, les avis et votre Banque de médias privée.</p></div><button type="button" class="secondary-button" data-action="admin-refresh">Actualiser</button></section>
  ...
  function mainMenuActionsHtml(mode) {
    const iconClass = mode === 'desktop' ? 'desktop-nav-icon' : 'desktop-nav-icon';
    return `
      <button type="button" class="menu-review-action" data-action="menu-open-review"><span class="${iconClass}" aria-hidden="true">★</span>&nbsp;&nbsp;Donner votre avis</button>
      <button type="button" class="menu-logout-action" data-action="logout"><span class="${iconClass}" aria-hidden="true">↪</span>&nbsp;&nbsp;Se déconnecter</button>`;
  }

  function renderNavigation() {
    desktopNav.innerHTML = `${navItems.map((item) => navButton(item, 'desktop')).join('')}${mainMenuActionsHtml('desktop')}`;
    drawerNav.innerHTML = `${navItems.map((item) => navButton(item, 'drawer')).join('')}${mainMenuActionsHtml('drawer')}`;
  ...
  function secondaryBackButton(fallbackPage = 'home') {
    return `<div class="page-back-row"><button type="button" class="page-back-button" data-action="go-back" data-fallback-page="${escapeHtml(fallbackPage)}" aria-label="Revenir à la page précédente"><span aria-hidden="true">←</span><span>Retour</span></button></div>`;
  }
  ...
  function renderHome() {
    ...
    return `
      <div class="page-stack home-refresh">
        ...
        <button id="homeReviewSummary" type="button" class="home-review-summary" data-action="open-review-summary" hidden>
          <span class="home-review-star" aria-hidden="true">★</span>
          <span class="home-review-copy"><strong data-review-average>—/5</strong><small data-review-count>Avis utilisateurs</small></span>
          <span class="home-review-label">Avis recueillis dans Animoa</span>
  ...
  function healthViewData(filter = currentHealthFilter) {
    ...
    const selectedGroup = resolveHealthGroup(filter);
    const items = allItems
      .filter((item) => selectedGroup.id === 'all' || selectedGroup.types.includes(item.type))
      .sort((a, b) => healthDateTimeKey(b).localeCompare(healthDateTimeKey(a)));
    const counts = HEALTH_GROUPS.reduce((result, group) => {
      result[group.id] = group.id === 'all'
        ? allItems.length
        : allItems.filter((item) => group.types.includes(item.type)).length;
      return result;
    }, {});
    return {
      allItems,
      selectedGroup,
      selectedType: selectedGroup.id,
      items,
      counts,
      selectedLabel: selectedGroup.title
    };
  }

  function healthEmptyState(group) {
    const states = {
      documents: {
        title: 'Aucun document',
        text: 'Ajoutez une ordonnance, un compte rendu ou un résultat d’analyse.',
        action: '<button type="button" class="secondary-button health-empty-action" data-add="health-document"><span aria-hidden="true">＋</span> Ajouter un document</button>',
        icon: HEALTH_GROUPS.find((item) => item.id === 'documents')?.icon || ''
      },
      agenda: {
        title: 'Aucun événement prévu',
        text: 'Ajoutez un rendez-vous, un vaccin ou un rappel à venir.',
        action: '<button type="button" class="secondary-button health-empty-action" data-add="appointment"><span aria-hidden="true">＋</span> Ajouter un rendez-vous</button>',
        icon: HEALTH_GROUPS.find((item) => item.id === 'agenda')?.icon || ''
      },
      care: {
        title: 'Aucun soin enregistré',
        text: 'Ajoutez un traitement, un médicament ou une analyse.',
        action: '<button type="button" class="secondary-button health-empty-action" data-add="health"><span aria-hidden="true">＋</span> Ajouter un soin</button>',
        icon: HEALTH_GROUPS.find((item) => item.id === 'care')?.icon || ''
      },
      all: {
        title: 'Carnet de santé vide',
        text: 'Ajoutez une première information pour commencer le suivi.',
        action: '<button type="button" class="secondary-button health-empty-action" data-add="health"><span aria-hidden="true">＋</span> Ajouter une information</button>',
        icon: HEALTH_GROUPS.find((item) => item.id === 'all')?.icon || ''
      }
    };
    const state = states[group.id] || states.all;
    return `<div class="health-empty-state"><div class="health-empty-icon">${state.icon}</div><strong>${state.title}</strong><p>${state.text}</p>${state.action}</div>`;
  }

  function healthListMarkup(view) {
    return view.items.length
      ? `<article class="card card-pad health-records-card"><div class="list">${view.items.map(renderHealthItem).join('')}</div></article>`
      : `<article class="card health-empty-card">${healthEmptyState(view.selectedGroup)}</article>`;
  }
  ...
  function renderHealth() {
    ...
    return `
      <div class="page-stack health-page">
        ${pageHeader('Santé', '', 'health')}
        <section class="health-main-tabs-panel" aria-label="Rubriques du carnet de santé">
          <div class="section-tabs health-tabs" id="healthTabs" role="tablist">${HEALTH_GROUPS.map((group) => {
            const count = view.counts[group.id] || 0;
            const active = view.selectedType === group.id;
            return `<button type="button" class="tab-chip health-main-tab ${active ? 'active' : ''}" data-health-filter="${group.id}" aria-selected="${active}" aria-label="${escapeHtml(group.label)} — ${count} élément${count > 1 ? 's' : ''}" title="${escapeHtml(group.label)}" role="tab"><span class="health-tab-icon">${group.icon}</span><span class="health-tab-label">${escapeHtml(group.label)}</span><small>${count}</small></button>`;
          }).join('')}</div>
        </section>
        <section class="health-content-section">
          <div class="card-title-row health-section-heading"><h2 id="healthListTitle">${escapeHtml(view.selectedLabel)}</h2><span id="healthListCount" class="summary-note">${view.items.length} élément${view.items.length > 1 ? 's' : ''}</span></div>
          <div id="healthList">${healthListMarkup(view)}</div>
        </section>
      </div>`;
  }
  ...
  function applyHealthFilter(filter) {
    ...
    if (list) list.innerHTML = healthListMarkup(view);

    [listTitle, listCount, list].forEach((node) => translateTree(node));
  }
  ...
  function renderHealthItem(item) {
    ...
    if (isDocument) {
      return `
        <div class="list-item record-list-item document-record-item">
          <div class="list-icon">${typeIcon(item.type)}</div>
          <div><p class="list-title">${escapeHtml(item.title)}</p><p class="list-subtitle">Document · ${escapeHtml(item.note || item.attachmentName || 'Aucun détail')}</p></div>
          <div class="record-trailing">
            <div class="list-value document-list-value">Document du ${formatDate(item.date, { short: true })}</div>
            <button type="button" class="record-menu-button" data-action="show-health-record" data-record-id="${item.id}" aria-label="Modifier ou supprimer ${escapeHtml(item.title)}">${recordMenuIcon()}</button>
          </div>
        </div>`;
    }
    ...
    return `
      <div class="list-item record-list-item">
        <div class="list-icon">${typeIcon(item.type)}</div>
        <div><p class="list-title">${escapeHtml(item.title)}</p><p class="list-subtitle">${escapeHtml(type)} · ${escapeHtml(item.professional || item.note || 'Aucun détail')}</p></div>
        <div class="record-trailing">
      ...
      <div class="list-item record-list-item reminder-list-item ${isRead ? 'is-read' : ''}">
        ...
        <div class="record-trailing reminder-trailing">
          ...
          <div class="reminder-actions">
            ...
            <button type="button" class="record-menu-button" data-action="show-health-record" data-record-id="${item.id}" aria-label="Voir ${escapeHtml(item.title)}">${recordMenuIcon()}</button>
          </div>
        </div>
      </div>`;
  }

  const EXPENSE_CHART_COLORS = Object.freeze(['#20b9b0', '#ff7f78', '#f3b657', '#9189dc', '#6ca7d8', '#79b98c']);

  function expenseBreakdown(items) {
    const byCategory = items.reduce((groups, item) => {
      const category = String(item.category || 'Autre').trim() || 'Autre';
      if (!groups[category]) groups[category] = { category, value: 0, count: 0 };
      groups[category].value += Number(item.amount || 0);
    ...
    const total = all.reduce((amount, entry) => amount + entry.value, 0);
    const visible = all.length > 4
      ? [...all.slice(0, 3), {
          category: 'Autres',
          value: all.slice(3).reduce((amount, entry) => amount + entry.value, 0),
          count: all.slice(3).reduce((count, entry) => count + entry.count, 0)
        }]
      : [...all];
    ...
    visible.forEach((entry, index) => {
      ...
      entry.color = EXPENSE_CHART_COLORS[index % EXPENSE_CHART_COLORS.length];
      entry.start = cursor;
    ...
    return { all, visible, total };
  }

  function expenseDonutGradient(entries) {
    if (!entries.length) return 'conic-gradient(var(--surface-soft) 0 100%)';
    return `conic-gradient(${entries.map((entry) => `${entry.color} ${entry.start.toFixed(3)}% ${entry.end.toFixed(3)}%`).join(', ')})`;
  }
  ...
  function renderExpenses() {
    ...
    const breakdown = expenseBreakdown(yearItems);
    const donutGradient = expenseDonutGradient(breakdown.visible);

    return `
      <div class="page-stack expenses-page">
        ${pageHeader('Dépenses', '', 'expense')}
        <section class="summary-grid expenses-summary-grid">
          ...
          ${summaryCard('Cette année', formatCurrency(breakdown.total), 'Total annuel', 'expenses')}
        </section>
        <article class="card card-pad expense-breakdown-card">
          <div class="card-title-row"><h2>Répartition ${year}</h2></div>
          ${breakdown.visible.length ? `
            <div class="expense-breakdown-main">
              <div class="expense-donut" style="--expense-gradient:${donutGradient}" role="img" aria-label="Répartition des dépenses de ${year}">
                <div class="expense-donut-center"><strong>${formatCurrency(breakdown.total)}</strong><span>au total</span></div>
              </div>
              <div class="expense-breakdown-legend">
                ${breakdown.visible.map((entry) => `
                  <div class="expense-legend-row">
                    <span class="expense-legend-dot" style="--expense-color:${entry.color}" aria-hidden="true"></span>
                    <span class="expense-legend-label">${escapeHtml(entry.category)}</span>
                    <strong>${formatCurrency(entry.value)}</strong>
                    <span class="expense-legend-percent">${Math.round(entry.percent)} %</span>
                  </div>`).join('')}
              </div>
            </div>
            <details class="expense-breakdown-details">
              <summary><span>Voir le détail</span><span class="expense-details-chevron" aria-hidden="true">›</span></summary>
              <div class="expense-breakdown-full">
                ${breakdown.all.map((entry) => `
                  <div class="expense-detail-row">
                    <span class="expense-legend-dot" style="--expense-color:${entry.color}" aria-hidden="true"></span>
                    <span><strong>${escapeHtml(entry.category)}</strong><small>${entry.count} dépense${entry.count > 1 ? 's' : ''}</small></span>
                    <span>${formatCurrency(entry.value)}<small>${Math.round(entry.percent)} % du total</small></span>
                  </div>`).join('')}
              </div>
            </details>` : '<div class="expense-breakdown-empty"><span aria-hidden="true">€</span><strong>Aucune dépense cette année</strong><p>Les catégories apparaîtront ici dès qu’une dépense sera ajoutée.</p></div>'}
        </article>
        <article class="card card-pad expense-history-card">
          ...
          <div class="list" style="margin-top:14px">
            ${items.length ? items.map((item) => `
              <div class="list-item record-list-item"><div class="list-icon">${typeIcon(item.category)}</div><div><p class="list-title">${escapeHtml(item.category)}</p><p class="list-subtitle">${escapeHtml(item.note || 'Sans description')} · ${formatDate(item.date)}</p></div><div class="record-trailing"><div class="list-value">${formatCurrency(item.amount)}</div><button type="button" class="record-menu-button" data-action="show-expense-record" data-record-id="${item.id}" aria-label="Modifier ou supprimer cette dépense">${recordMenuIcon()}</button></div></div>`).join('') : emptyList('Aucune dépense enregistrée.')}
          </div>
        </article>
      </div>`;
  }
  ...
  function renderAddPet() {
    ...
    return `
      <div class="page-stack quick-pet-page">
        <div class="page-header quick-pet-page-header">
          <div><p class="eyebrow">Ajouter</p><h1>Ajouter un animal</h1></div>
          <button type="button" class="secondary-button quick-pet-back" data-page="animals">Retour</button>
        </div>
        <article class="card card-pad quick-pet-card">
          <p class="quick-pet-intro">Créez sa fiche maintenant. Les autres informations pourront être ajoutées ensuite.</p>
          <form id="quickPetForm" class="form-grid professional-form quick-pet-form" autocomplete="off">
            <div class="form-row">
              <label for="quickPetName">Nom de l’animal</label>
              <input id="quickPetName" name="name" required maxlength="60" placeholder="Ex. Milo" autofocus />
            </div>
            <fieldset class="quick-pet-species-fieldset">
              <legend>Espèce</legend>
              <div class="quick-pet-species-grid">
                ${speciesChoices.map((choice, index) => `
                  <label class="quick-pet-species-choice">
                    <input type="radio" name="species" value="${choice.value}" ${index === 0 ? 'checked' : ''} />
                    <span class="quick-pet-species-tile"><span class="quick-pet-species-icon" aria-hidden="true">${choice.icon}</span><strong>${choice.value}</strong></span>
                  </label>`).join('')}
              </div>
            </fieldset>
            <button class="primary-button form-submit-button quick-pet-submit" type="submit"><img src="assets/animoa-icon-official.png" alt="" width="331" height="378" decoding="async" />Créer la fiche</button>
            <button type="button" class="quick-pet-cancel" data-page="animals">Annuler</button>
          </form>
        </article>
      </div>`;
  }
  ...
  function renderSettings() {
    ...
    const notificationsContent = `
      <article class="card card-pad settings-section notification-settings-card settings-inner-card">
        ...
        <div class="notification-toggle-list">
          <label class="settings-toggle-row"><span><strong>Rendez-vous</strong><small>Notifications : 24 h et 2 h avant. E-mail : dans les 24 h précédentes.</small></span><input type="checkbox" data-setting-toggle="notifyAppointments" ${settings.notifyAppointments ? 'checked' : ''}></label>
          <label class="settings-toggle-row"><span><strong>Vaccins</strong><small>Notifications : 30 j, 7 j et jour J. E-mail : 7 j avant.</small></span><input type="checkbox" data-setting-toggle="notifyVaccines" ${settings.notifyVaccines ? 'checked' : ''}></label>
          <label class="settings-toggle-row"><span><strong>Traitements</strong><small>Notification à l’échéance. E-mail dans les 24 h précédentes.</small></span><input type="checkbox" data-setting-toggle="notifyTreatments" ${settings.notifyTreatments ? 'checked' : ''}></label>
    ...
    const helpContactContent = `
      <div class="settings-help-content">
        <article class="card card-pad settings-help-card settings-compact-row settings-inner-card">
          <div><p class="eyebrow">Assistance</p><h2>Besoin d’aide ?</h2><p>Consultez le guide ou envoyez-nous un message directement depuis Animoa.</p></div>
          <div class="settings-help-actions"><button type="button" class="secondary-button" data-page="help">Ouvrir le guide</button><button type="button" class="primary-button" data-page="contact">Nous contacter</button></div>
        </article>
        <aside class="settings-social-row" aria-label="Réseaux sociaux Animoa">
          <div class="settings-social-copy"><strong>Suivez Animoa</strong><small>Actualités, conseils et coulisses.</small></div>
          <div class="settings-social-links">
            <a class="settings-social-link" href="https://www.tiktok.com/@animoa_app" target="_blank" rel="noopener noreferrer" aria-label="Ouvrir le profil TikTok Animoa">
              <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M16.6 3c.5 2.7 2 4.3 4.4 4.5v3.6a8.7 8.7 0 0 1-4.4-1.3v6.4a6.2 6.2 0 1 1-5.4-6.1v3.7a2.7 2.7 0 1 0 1.8 2.5V3h3.6Z"/></svg>
              <span>TikTok</span>
            </a>
            <a class="settings-social-link" href="https://www.facebook.com/animoa.fr" target="_blank" rel="noopener noreferrer" aria-label="Ouvrir la page Facebook Animoa">
              <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13.8 21v-8h2.8l.4-3h-3.2V8.1c0-.9.3-1.5 1.6-1.5H17V3.9c-.7-.1-1.5-.2-2.3-.2-2.3 0-3.9 1.4-3.9 4V10H8.2v3h2.6v8h3Z"/></svg>
              <span>Facebook</span>
            </a>
          </div>
        </aside>
    ...
    const dataPrivacyContent = `
      <article class="card card-pad danger-zone settings-compact-row settings-inner-card">
        <div><p class="eyebrow danger-eyebrow">Données enregistrées</p><h2>Effacer toutes les données</h2><p>Cette action vide le carnet et supprime les photos, mais conserve le compte et l’adresse de connexion.</p></div>
        <button type="button" class="danger-button" data-action="reset-data">Effacer toutes les données</button>
      </article>
      ${user && cloudEnabled ? `<article class="card card-pad danger-zone account-deletion-zone settings-compact-row settings-inner-card">
        <div><p class="eyebrow danger-eyebrow">Suppression du compte</p><h2>Supprimer définitivement mon compte</h2><p>Cette action supprime le compte Animoa, le carnet, les documents et les photos. Elle est irréversible.</p></div>
      ...
      <article class="card card-pad settings-support-card settings-compact-row settings-inner-card">
        <div><p class="eyebrow">Confidentialité</p><h2>Comprendre l’utilisation de vos données</h2><p>Consultez les informations de confidentialité et les mentions légales d’Animoa.</p></div>
        <div class="settings-support-actions"><button type="button" class="secondary-button" data-page="privacy">Confidentialité</button><button type="button" class="secondary-button" data-page="legal">Mentions légales</button></div>
      </article>
      <div class="settings-legal-row"><span>Version ${APP_VERSION}</span></div>`;
    ...
    return `
      <div class="page-stack settings-page">
        ...
        <div class="settings-accordion-list" aria-label="Rubriques des paramètres">
          ${settingsAccordion({ id: 'settingsAccount', icon: '👤', title: 'Mon compte', subtitle: 'Connexion et synchronisation', content: accountContent })}
          ${settingsAccordion({ id: 'settingsNotifications', icon: '🔔', title: 'Notifications', subtitle: 'Rappels et calendrier', content: `${notificationsContent}${calendarSettingsCardHtml()}` })}
          ...
          ${settingsAccordion({ id: 'settingsHelp', icon: '❔', title: 'Aide et contact', subtitle: 'Guide, contact et réseaux sociaux', content: helpContactContent })}
          ${settingsAccordion({ id: 'settingsPrivacy', icon: '🛡️', title: 'Données et confidentialité', subtitle: 'Gestion et protection des données', content: dataPrivacyContent })}
        </div>
      </div>`;
  }
    ...
    return `
      <div class="page-stack legal-page">
        ...
        <article class="card card-pad legal-card">
          ...
          <h2>Conservation et suppression</h2><p>Les données sont conservées tant que le compte est utilisé. Elles peuvent être effacées depuis les paramètres ou sur demande. La suppression définitive du compte supprime également l’avis interne associé. Une demande peut aussi être adressée à <a href="mailto:${SUPPORT_EMAIL}">${SUPPORT_EMAIL}</a>.</p>
  ...
  function closeModal() {
    const shouldRecordReviewLater = reviewPromptActive && reviewPromptMode === 'automatic' && !reviewPromptRecording;
    reviewPromptActive = false;
    if (shouldRecordReviewLater) {
      recordReviewAction('later', { close: false, silent: true }).catch((error) => console.warn('Report de demande d’avis non enregistré', error));
    }
    ...
    modalCloseTimer = setTimeout(() => {
      ...
      modalBody.innerHTML = '';
      modal.classList.remove('modal-centered', 'modal-sensitive');
      reviewPromptMode = 'automatic';
      modalCloseTimer = null;
      // Ne jamais recontrôler immédiatement après la fermeture : cela pouvait
      // rouvrir la demande d’avis en boucle avant la synchronisation Supabase.
    }, 220);
  }
  ...
  function openAddForm(type) {
    ...
    const forms = {
      ...
      health: healthForm,
      'health-document': () => healthForm(null, 'Document'),
      expense: expenseForm,
      weight: weightForm,
      memory: memoryForm
    };
    const titles = { appointment: 'Nouveau rendez-vous', health: 'Information de santé', 'health-document': 'Nouveau document', expense: 'Nouvelle dépense', weight: 'Nouveau poids', memory: 'Nouveau souvenir' };
  ...
  function healthForm(item = null, initialType = 'Vaccin') {
    ...
    const status = isDocument ? 'done' : (item?.status || 'planned');
    const date = item?.date || todayIso();
    const min = !isDocument && status === 'planned' ? todayIso() : '';
    const max = isDocument || status === 'done' ? todayIso() : '';
    const isAppointment = selectedType === 'Rendez-vous';
    const usesCalendar = healthTypeUsesCalendar(selectedType);
    const showTime = !isDocument && (status === 'planned' || isAppointment);
    ...
    return `<form id="healthForm" class="form-grid professional-form" data-editing="${item?.id || ''}">
      ...
      <div class="form-columns health-schedule-fields"><div class="form-row health-date-field"><label id="healthDateLabel" for="healthDate">${isDocument ? 'Date du document' : 'Date'}</label>${dateInputControl({ id: 'healthDate', name: 'date', value: date, min, max, required: true, describedBy: 'healthDateHelp' })}<span id="healthDateHelp" class="form-help" hidden></span></div><div class="form-row health-status-field" id="healthStatusRow" ${isDocument ? 'hidden' : ''}><label for="healthStatus">État</label><select id="healthStatus" name="status" ${isDocument ? 'disabled' : ''}><option value="planned" ${optionSelected(status, 'planned')}>À venir</option><option value="done" ${optionSelected(status, 'done')}>Effectué</option></select></div></div>
      <div class="form-row" id="healthTimeRow" ${showTime ? '' : 'hidden'}><label id="healthTimeLabel" for="healthTime">${timeLabel}</label><input id="healthTime" name="time" type="text" inputmode="numeric" maxlength="5" autocomplete="off" placeholder="HH:MM" value="${escapeHtml(!isDocument && validTime(item?.time) ? item.time : '')}" ${showTime ? (timeRequired ? 'required' : '') : 'disabled'} aria-describedby="healthTimeHelp" /><span id="healthTimeHelp" class="form-help" hidden>${timeHelp}</span></div>
      ...
      <label class="checkbox-row" id="healthReminderRow" ${isDocument ? 'hidden' : ''}><input name="reminder" type="checkbox" ${isDocument ? 'disabled' : ''} ${!isDocument && (item ? item.reminder : true) ? 'checked' : ''} /> Créer un rappel dans Animoa</label>
      <label class="checkbox-row calendar-option-row" id="calendarOptionRow" ${!isDocument && usesCalendar && status === 'planned' ? '' : 'hidden'}><input name="offerCalendar" type="checkbox" ${isDocument ? 'disabled' : ''} ${!isDocument && (item ? item.offerCalendar !== false : true) ? 'checked' : ''} /> ${googleCalendarConnected() && settings.calendarAutoSync ? 'Synchroniser automatiquement avec Google Agenda' : 'Ajouter cette action de santé à mon calendrier'}</label>
  ...
  async function handleQuickPetSubmit(form) {
    const formData = new FormData(form);
    ...
    try {
      const name = requiredText(formData.get('name'), 'le nom de l’animal');
      const species = String(formData.get('species') || 'Chien');
      if (!['Chien', 'Chat', 'Oiseau', 'Lapin', 'Autre'].includes(species)) throw new Error('Choisissez une espèce valide.');
      ...
      data = snapshot;
      renderPage();
      showToast(error.message || 'Impossible de créer cet animal.');
    } finally {
  ...
  async function handleFormSubmit(form) {
    ...
    if (form.id === 'healthForm' && googleCalendarConnected() && settings.calendarAutoSync) {
      ...
      const nextStatus = nextType === 'Document' ? 'done' : String(formData.get('status') || 'planned');
    ...
    try {
      if (form.id === 'healthForm') {
        ...
        const isDocument = type === 'Document';
        const status = isDocument ? 'done' : String(formData.get('status') || 'planned');
        const date = parseFrenchDate(formData.get('date'), isDocument ? 'La date du document' : 'La date', { required: true });
        const isAppointment = type === 'Rendez-vous';
        const showsTime = !isDocument && (status === 'planned' || isAppointment);
        const timeRequired = false;
        const time = showsTime ? parseHealthTime(formData.get('time'), timeRequired) : '';
        validateHealthDate(status, date);
        if (!isDocument && validTime(time)) validateHealthDateTime(status, date, time);
        ...
        const record = {
          ...
          reminder: isDocument ? false : formData.get('reminder') === 'on',
          offerCalendar: isDocument ? false : formData.get('offerCalendar') === 'on',
        ...
        currentHealthFilter = healthGroupForType(record.type).id;
        saveData();
  ...
  function showHealthRecord(recordId) {
    ...
    const isDocument = normalizeHealthType(item.type) === 'Document';
    const status = item.status === 'planned' ? (isPlannedHealthOverdue(item) ? 'En retard' : 'À venir') : 'Effectué';
    const timeDetail = !isDocument && validTime(item.time) ? `<span>Heure</span><strong>${item.time}</strong>` : '';
    const locationDetail = item.location ? `<span>Lieu</span><strong>${escapeHtml(item.location)}</strong>` : '';
    const statusDetail = isDocument ? '' : `<span>État</span><strong>${status}</strong>`;
    const professionalDetail = item.professional ? `<span>Professionnel</span><strong>${escapeHtml(item.professional)}</strong>` : '';
    const isCalendarAppointment = !isDocument && healthTypeUsesCalendar(item.type) && item.status === 'planned';
    ...
    openModal(item.title, `
      <div class="record-detail">
        ...
        <div class="detail-grid"><span>${isDocument ? 'Date du document' : 'Date'}</span><strong>${formatDate(item.date)}</strong>${timeDetail}${locationDetail}${statusDetail}${professionalDetail}</div>
  ...
  function syncHealthTimeField() {
    ...
    const statusRow = document.getElementById('healthStatusRow');
    ...
    const wasDocument = status.disabled;
    if (isDocument) status.value = 'done';
    else if (wasDocument) status.value = 'planned';
    status.disabled = isDocument;
    if (statusRow) statusRow.hidden = isDocument;
    const showTime = !isDocument && (status.value === 'planned' || isAppointment);
    const timeRequired = false;
    row.hidden = !showTime;
    ...
    const dateLabel = document.getElementById('healthDateLabel');
    if (dateLabel) dateLabel.textContent = translateText(isDocument ? 'Date du document' : 'Date');
    const reminderRow = document.getElementById('healthReminderRow');
    const reminderInput = reminderRow?.querySelector('input[name="reminder"]');
    if (reminderRow) reminderRow.hidden = isDocument;
    if (reminderInput) {
      reminderInput.disabled = isDocument;
      if (isDocument) reminderInput.checked = false;
    }
    const calendarRow = document.getElementById('calendarOptionRow');
    const calendarInput = calendarRow?.querySelector('input[name="offerCalendar"]');
    if (calendarRow) calendarRow.hidden = isDocument || !(usesCalendar && status.value === 'planned');
    if (calendarInput) {
      calendarInput.disabled = isDocument;
      if (isDocument) calendarInput.checked = false;
    }
    syncHealthDateRules(true);
  }
  ...
  function requestDeleteAccount() {
    const user = window.AnimoaAuth?.getUser?.();
    if (!user || window.AnimoaAuth?.isLocalPreview?.()) {
      showToast('Aucun compte connecté ne peut être supprimé.');
      return;
    }
    openModal('Supprimer mon compte ?', `
      <div class="delete-confirmation account-delete-confirmation">
        <div class="delete-warning-icon">!</div>
        <p><strong>Toutes vos données Animoa seront supprimées.</strong></p>
        <p>Votre compte, vos animaux, vos informations de santé, vos dépenses, vos souvenirs, vos documents et vos photos seront effacés définitivement.</p>
        <p class="account-delete-email">Compte concerné : <strong>${escapeHtml(user.email || '')}</strong></p>
        <div class="delete-actions">
          <button type="button" class="secondary-button" data-action="close-modal">Annuler</button>
          <button type="button" class="danger-button" data-action="continue-delete-account">Continuer</button>
        </div>
      </div>
    `, 'Suppression du compte', { sensitive: true });
  }

  function requestFinalDeleteAccountConfirmation() {
    const user = window.AnimoaAuth?.getUser?.();
    if (!user || window.AnimoaAuth?.isLocalPreview?.()) {
      closeModal();
      showToast('Aucun compte connecté ne peut être supprimé.');
    ...
    openModal('Êtes-vous vraiment sûr ?', `
      <div class="delete-confirmation account-delete-confirmation">
        <div class="delete-warning-icon">!</div>
        <p><strong>Cette action est irréversible.</strong></p>
        <p>Après confirmation, votre compte et toutes ses données ne pourront plus être récupérés.</p>
        <p id="deleteAccountError" class="form-error account-delete-error" role="alert"></p>
        <div class="delete-actions">
          <button type="button" class="secondary-button" data-action="request-delete-account">Non, revenir</button>
          <button type="button" class="danger-button" data-action="confirm-delete-account">Oui, supprimer définitivement</button>
        </div>
      </div>
    `, 'Dernière confirmation', { sensitive: true });
  }
  ...
  function clearDeletedAccountLocalData(userId) {
    ...
    localStorage.removeItem(`${UPDATED_AT_KEY}:${userId}`);
    localStorage.removeItem('animoa_legacy_migration_owner');
  }

  async function clearDeletedAccountLocalMedia() {
    for (const ref of mediaRefsFrom(data)) {
  ...
  async function deleteCurrentAccount(button) {
    const errorNode = document.getElementById('deleteAccountError');
    const client = window.AnimoaAuth?.getClient?.();
    const user = window.AnimoaAuth?.getUser?.();
    if (!client || !user || window.AnimoaAuth?.isLocalPreview?.()) {
      if (errorNode) errorNode.textContent = 'La suppression du compte nécessite une connexion active.';
      return;
    }

    const originalLabel = button?.textContent || 'Supprimer définitivement';
    if (button) {
    ...
    try {
      const { data: response, error } = await client.functions.invoke('animoa-delete-account', {
        body: { confirmation: 'SUPPRIMER' }
      });
      if (error) throw error;
      if (!response?.ok) throw new Error(response?.error || 'La suppression du compte n’a pas pu être terminée.');

      clearDeletedAccountLocalData(user.id);
      await clearDeletedAccountLocalMedia();
      try { await window.AnimoaAuth?.clearDeletedAccountSession?.(); } catch {}
      location.replace('/');
    } catch (error) {
      console.error('Suppression du compte Animoa impossible', error);
      if (errorNode) errorNode.textContent = error.message || 'Suppression impossible pour le moment. Réessayez dans quelques instants.';
      if (button) {
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (filter) {
      ...
      applyHealthFilter(HEALTH_GROUP_IDS.has(filter) ? filter : healthGroupForType(filter).id);
      return;
    }
    ...
    if (action === 'admin-tab') { adminActiveTab=target.dataset.adminTab||'users'; adminStatusFilter='all'; renderPage(); if (adminActiveTab === 'media' && !adminMediaDriveStatus.loaded) await loadAdminMediaDriveStatus({ quiet: true }); }
    ...
    if (action === 'continue-delete-account') requestFinalDeleteAccountConfirmation();
    if (action === 'confirm-delete-account') await deleteCurrentAccount(target);
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (!event.target.matches('#healthForm, #expenseForm, #weightForm, #memoryForm, #petForm, #quickPetForm')) return;
    event.preventDefault();
    if (event.target.id === 'quickPetForm') await handleQuickPetSubmit(event.target);
    else await handleFormSubmit(event.target);
  });
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    else queueReviewPromptCheck(700);
  });
  window.addEventListener('focus', () => queueReviewPromptCheck(700));
